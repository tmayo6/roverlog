...
proc Net_Send_Kernel { what peerno msg } {
  ...
  # send operating frequency
    ...
    if { $r != $stuff(lastfrqsend) } {
      Debug "Net_Send_Kernel" \
        "sending freq to $peername."

      Peer_Puts $peerno $r
      set stuff(lastfrqsend) "$r"
    }
...
proc Web_Lookup { } {
  ...
  "ARRL" {
    eval exec [auto_execok start] [list "http://www.arrl.org/fcc/fcclook.php3?call=$call"] &
  }
  default {
    eval exec [auto_execok start] [list "http://www.qrz.com/detail/$call"] &
  }
  }
}
...
proc Redraw_Score { } {
  ...
  if { $::setting(rules) != "grid6" && $::setting(rules) != "dist" } {
    ...
    foreach b $stuff(loglist) {
      ...
      if { $distkm < 1 } {
        Debug "Redraw_Score" "Runty QSO. Skipping."
        continue
      }
    ...
    $windows(scoretext) insert insert "\nTotal 6-Digit Distance-Based Score: $totaldistscore"
